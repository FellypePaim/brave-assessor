...
export default function WhatsAppLinkCard({ userId }: WhatsAppLinkCardProps) {
  ...
  const generateCode = async () => {
    ...
    if (error) {
      ...
      await fetchLink();
      toast({ title: "Código gerado!", description: "Envie o código no WhatsApp do Nox" });
    }
    setLoading(false);
  };
      ...
      {isLinked ? (
        <div className="space-y-4">
          <div className="bg-accent/50 border border-border rounded-xl p-4">
            <p className="text-sm text-foreground font-medium">📱 Número vinculado</p>
        ...
        <div className="space-y-4">
          <div className="bg-accent/50 border border-border rounded-xl p-6 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1 mb-4">
              Gere um código, envie no WhatsApp do Nox e pronto!
              Registre transações apenas enviando mensagens.
            </p>
